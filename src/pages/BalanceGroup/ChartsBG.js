...
const OutInputMonthGraphic = () => {
  ...
  useEffect(() => {
    setLoading(true);
    if (globalState.balance_index !== "") {
      ServicesBG.getResImbalanceByYear(globalState.balance_index, year)
        .then((result) => {
          setOutMonth(result);
          setLoading(false);
        })
          ...
          <Paper elevation={1} className={classes.paperShadow}>
            <Grid container>
              <Grid item lg={6} md={12} sm={12} xl={6} xs={12}>
                <Box>
                  <Typography className={classes.chartTitle}>
                    График суммарных помесячных показаний согласно приборам
                    учета, кВтч
                  </Typography>
                  <DisplayBarChart
                    type={input_month}
              ...
              <Grid item lg={6} md={12} sm={12} xl={6} xs={12}>
                <Box>
                  <Typography className={classes.chartTitle}>
                    График суммарных помесячных показаний гарантирующих
                    поставщиков, кВтч
                  </Typography>
                  <DisplayBarChart
                    type={out_psk_month}
